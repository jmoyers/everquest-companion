...
export interface QuestListState {
  ...
  /** Clear every narrowing control on the Quests tab; sort and the stored Show all choice remain. */
  resetFilters: () => void
  /**
...
function questNavigation(set: {
  ...
  setFavoritesOnly: (v: boolean) => void
  resetPaging: () => void
}): Pick<QuestListState, 'resetFilters' | 'revealQuest' | 'showClassQuests'> {
